...
import { Filter, ArrowUpDown, ChevronRight, PackageCheck } from 'lucide-react';
...
const PacotesConfirmados = () => {
  ...
  useEffect(() => {
    if (location.state?.itens && location.state?.status) {
      const itensFiltrados = location.state.itens.filter(
        (item: EstoqueItem) => item.status === 'confirmed_input_warehouse'
      );
  ...
  return (
    <MobileLayout title="Pacotes no estoque" showBackButton>
            ...
            <Sheet open={ordenacaoAberta} onOpenChange={setOrdenacaoAberta}>
              ...
              <SheetContent side="top" className="h-auto">
                ...
                <div className="space-y-4 mt-4">
                  ...
                  <Button onClick={aplicarOrdenacao} className="w-function">
                    Aplicar Ordenação
                  </Button>
                </div>
              </SheetContent>
        ...
        <div className="flex-1 p-4 space-y-3">
          {pacotes.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-500">Nenhum pacote confirmado encontrado</p>
            </div>
          ) : (
            pacotes.map((pacote) => (
              <button
...
export default PacotesConfirmados;
