...
import { Filter, ArrowUpDown, ChevronRight, PackageOpen } from 'lucide-react';
...
const PacotesBipados = () => {
  ...
  useEffect(() => {
    if (location.state?.itens && location.state?.status) {
      const itensFiltrados = location.state.itens.filter(
        (item: EstoqueItem) => item.status === 'input_warehouse'
      );
  ...
  return (
    <MobileLayout title="Pacotes bipados" showBackButton>
        ...
        <div className="flex-1 p-4 space-y-3">
          {pacotes.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-500">Nenhum pacote bipado encontrado</p>
            </div>
          ) : (
            pacotes.map((pacote) => (
              <button
