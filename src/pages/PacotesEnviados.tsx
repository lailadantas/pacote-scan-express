...
import { Filter, ArrowUpDown, ChevronRight, PackageMinus } from 'lucide-react';
...
const PacotesEnviados = () => {
  ...
  useEffect(() => {
    if (location.state?.itens && location.state?.status) {
      const itensFiltrados = location.state.itens.filter(
        (item: EstoqueItem) => item.status === 'confirmed_output_warehouse'
      );
  ...
  return (
    <MobileLayout title="Pacotes enviados" showBackButton>
        ...
        <div className="flex-1 p-4 space-y-3">
          {pacotes.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-500">Nenhum pacote enviado encontrado</p>
            </div>
          ) : (
            pacotes.map((pacote) => (
              <button
...
export default PacotesEnviados;
